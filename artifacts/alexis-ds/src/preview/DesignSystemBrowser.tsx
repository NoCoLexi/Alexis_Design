    ...
    <nav aria-label="Design system navigation" className="space-y-5 py-2">
      {showOverview ? (
        <button
          ...
          className="block w-full rounded-none px-2 py-2 text-left text-sm font-medium transition-colors hover:bg-muted aria-[current=true]:bg-primary aria-[current=true]:text-primary-foreground"
        >
          {OVERVIEW_ENTRY.name}
        </button>
      ) : null}
          ...
          <div className="mt-2 space-y-1 border-l pl-2">
            {group.entries.map((entry) => (
              <button
                ...
                className="block w-full rounded-none px-2 py-1.5 text-left text-sm transition-colors hover:bg-muted aria-[current=true]:bg-primary aria-[current=true]:text-primary-foreground"
              >
                {entry.name}
              </button>
            ))}
          ...
          <div className="pt-8">
            <Suspense
              fallback={
                <div
                  role="status"
                  className="rounded-none border bg-card p-6 text-sm text-muted-foreground"
                >
                  Loading preview…
                </div>
              }
