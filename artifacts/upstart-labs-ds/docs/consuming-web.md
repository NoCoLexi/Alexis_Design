# Consuming UpStart-Labs Design System in web apps

Read `artifacts/upstart-labs-ds/docs/AGENTS.md` first. This guide covers
...
`artifacts/upstart-labs-ds/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/upstart-labs-ds/styles.css";
```
...
`node_modules/@workspace/upstart-labs-ds/src/components` to `content`.
...
```tsx
import { Button } from "@workspace/upstart-labs-ds/components/ui/button";
import { cn } from "@workspace/upstart-labs-ds/lib/utils";
import {
  toast,
  useToast,
} from "@workspace/upstart-labs-ds/hooks/use-toast";
```
...
`@workspace/upstart-labs-ds/components/ui/button`. Run the app's typecheck
