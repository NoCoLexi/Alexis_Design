# Migrating Expo UI to UpStart-Labs Design System

Read `artifacts/upstart-labs-ds/docs/AGENTS.md` and
`artifacts/upstart-labs-ds/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/upstart-labs-ds/lib/native-theme` and
  `@workspace/upstart-labs-ds/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/upstart-labs-ds/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/upstart-labs-ds` and retains only
